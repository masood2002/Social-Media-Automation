...
export const triggerValidations = async (req, res, next) => {
  try {
    const name = req.body.name;

    // Check if the name field is empty or not a string
    ...
    // Regex to ensure the name only contains letters and spaces
    ...
    if (!validNameRegex.test(trimmedName)) {
      return res
        .status(422)
        .json(errorResponse(req.__("nameInvalidCharacters")));
    }
